...
# Intentar importar Google Ads, pero hacerlo opcional
try:
    from google.ads.googleads.client import GoogleAdsClient
    from google.ads.googleads.errors import GoogleAdsException
    GOOGLE_ADS_AVAILABLE = True
except ImportError:
    logger.warning("Google Ads library not available. Volume analysis features will be disabled.")
...
def get_google_ads_client():
    ...
    if not GOOGLE_ADS_AVAILABLE:
        logger.error("Google Ads library is not installed")
        return None
...
def get_keyword_search_volume(keywords, customer_id=None, language_id='1003', location_ids=['2840']):
    ...
    if not GOOGLE_ADS_AVAILABLE:
        return {
            'status': 'error',
            'error': 'La biblioteca de Google Ads no está disponible. Por favor, contacta al administrador.'
        }
